...
| `GET /v1/claim-verification/reviews`, `/summary`, `/{review_id}` | Page through ACL-filtered production claim samples, aggregate reviewer-visible metrics, and fetch evidence on demand |
| `POST /v1/claim-verification/reviews/{review_id}/label` | Submit a human verdict with optimistic revision checking |
| `GET /v1/claim-verification/reviews/export` | Page through reviewed rows in claim-verification eval format |
...
| `GET /v1/claim-verification/observations/summary` | Reviewer-only, tenant-scoped rollout window and operational readiness summary |
...
| `CLAIM_VERIFICATION_MODE` | `off` | Claim verifier rollout: `off`, observe-only `shadow`, or fail-closed `enforce` |
| `CLAIM_VERIFICATION_ENABLED` | `false` | Legacy fallback: `true` maps to `enforce` only when `CLAIM_VERIFICATION_MODE` is unset |
| `CLAIM_VERIFICATION_ROLLOUT_PERCENT` | `100` | Sticky cohort percentage promoted to the configured mode; range `0..100` |
| `CLAIM_VERIFICATION_ROLLOUT_SEED` | `cogdoc-v1` | Deterministic cohort seed; changing it intentionally rebuckets traffic |
| `CLAIM_VERIFICATION_OBSERVATION_RETENTION_DAYS` | `30` | Durable metadata-only observation retention, bounded to `1..365` days |
| `CLAIM_VERIFICATION_OBSERVATION_MAX_PER_TENANT` | `100000` | Hard observation-row cap per tenant |
| `CLAIM_VERIFICATION_OPERATIONAL_MIN_SAMPLES` | `200` | Executed non-off audits required for operational readiness |
| `CLAIM_VERIFICATION_OPERATIONAL_MAX_ERROR_RATE` | `0.02` | Maximum verifier error rate for operational readiness |
| `CLAIM_VERIFICATION_REVIEW_SAMPLE_PERCENT` | `0` | Opt-in deterministic percentage of audited claims copied to the human-review queue; sampled rows contain claim text and bounded cited evidence |
| `CLAIM_VERIFICATION_REVIEW_SAMPLE_SEED` | `cogdoc-review-v1` | Independent deterministic review-sampling seed |
| `CLAIM_VERIFICATION_REVIEW_RETENTION_DAYS` | `30` | Human-review row retention, bounded to `1..365` days |
| `CLAIM_VERIFICATION_REVIEW_MAX_PER_TENANT` | `10000` | Hard human-review row cap per tenant |
| `CLAIM_VERIFICATION_REVIEW_MAX_CLAIMS_PER_RESPONSE` | `5` | Maximum sampled claims from one completed response |
| `CLAIM_VERIFICATION_REVIEW_MAX_EVIDENCE_PER_CLAIM` | `6` | Maximum exact cited-evidence snapshots retained per sampled claim |
| `CLAIM_VERIFICATION_REVIEW_MAX_CHARS_PER_EVIDENCE` | `1600` | Character cap per retained evidence snapshot |
...
| `make eval-claim-verification` | Evaluate manually labeled semantic claim-verification results |
| `make eval-claim-verification-gate` | Gate unsafe acceptance, recall, latency, and per-path quality |
| `make eval-claim-verification-promote` | Atomically promote a passing claim-verification baseline |
...
### Semantic claim-verification release gate

Claim-audit metrics in the general quality report remain useful diagnostics. Strict release acceptance uses a separate, manually labeled `eval/claim_verification_eval.jsonl`. The real set is ignored by Git; the repository includes only the runnable `eval/claim_verification_eval.example.jsonl`. Every row requires a stable `id`, a `layer`, and an `expected_verdict` of `supported`, `unsupported`, `insufficient`, or `not_factual`. A recorded run can provide `actual_verdict`, or the row can embed a production `claim_audit`; use `claim_id` to select one claim from a multi-claim audit. Missing, errored, or malformed audits always produce a reject decision while reducing observability, so they cannot masquerade as correct predictions.

`make eval-claim-verification` reports a confusion matrix, support precision/recall, unsafe-claim acceptance, unobservable fail-closed behavior, and verifier latency. `make eval-claim-verification-gate` applies `eval/claim_verification_gate.json`: sample-maturity checks, Wilson confidence bounds for classification proportions, percentile-bootstrap latency bounds, QA/Summary/Compare slice thresholds, and regression checks against the last baseline. A mismatched evaluation-contract hash fails closed; with no historical baseline it runs only the absolute checks. `make eval-claim-verification-promote` atomically replaces `artifacts/reliability/claim-verification-baseline.json` only after every check passes; a rejected candidate leaves the accepted baseline untouched.
