...
- `data/kb/`: knowledge-base registry, source files, generation state, and ingest journal.
...
`COGDOC_ACCOUNT_AUTH_ENABLED=false` is a local-development compatibility mode. Credential-free owner access is granted only when the effective client address has been verified as loopback; an individual or team production installation must enable account authentication or configure explicit static principals. A deployment behind a reverse proxy must list the proxy networks in `COGDOC_TRUSTED_PROXY_CIDRS` and have the proxy overwrite `Forwarded` or `X-Forwarded-For`. Requests arriving through a trusted proxy without a valid client chain fail closed instead of inheriting the proxy process's loopback owner access. The module-level service then creates the authentication and resource-ACL tables in `COGDOC_DATA_DIR/state.db`; configured static API principals continue to work for service automation. Human login and invitation tokens are returned only to the caller and only their SHA-256 digests are stored. Passwords are salted, versioned scrypt hashes, with a 12-character minimum, bounded failure lockout, and configurable login-session/invitation TTLs.
...
For an existing static-principal installation, enable accounts as a staged migration rather than assuming principal records become users. There is no automatic conversion from `COGDOC_API_PRINCIPALS` to password accounts, and newly registered account workspaces receive new IDs. Before changing the flag, export an inventory of every tenant/KB ID and retain the old service principals during the migration; once ACL enforcement starts, KBs without a policy disappear from ordinary lists by design. A tenant-matched owner/admin service key must initialize each known pre-existing KB with `PATCH /v1/knowledge-bases/{kb}/access`; missing ACL rows deny access even to administrators. Use `GET /v1/knowledge-bases/{kb}/documents` to obtain stable `document_id` values before adding per-document policies or grants. To move a KB into a newly generated account workspace, create it in that workspace and re-ingest its original source documents through the supported API/CLI path; do not rewrite registry or ACL tenant IDs directly in SQLite. Remove legacy keys only after account workspaces, policies, memberships, quotas, traces, and Research artifacts have been verified.

This release uses `document_id = source-name-v1` metadata and the chunk-identity contract `source_sha256_name_page_span_local_v7_document_acl_parent_child_section_index_adaptive_blocks_cs600_ov60_min30_ctx160_strategy_adaptive-structural-v2`. Treat a change from an older contract as rebuild-required: rebuild every affected source's vector/BM25 generation through normal ingestion before considering the rollout complete. A SUBSET authorization is pushed into vector and BM25 selection before top-k and is checked again after fusion. Background Research persists the creator and frozen allowlist, revalidates current membership/ACLs around retrieval, and fails closed when access is revoked or the backend cannot enforce a subset.

The local identity implementation is intended for one writable CogDoc service instance sharing one protected data directory. Do not place independent `state.db` files behind a load balancer and expect sessions, invites, memberships, or ACL epochs to converge. Restrict filesystem permissions and encrypt backups because `state.db` contains password hashes, OIDC identity mappings and encrypted in-flight flows, SCIM state, and live-token digests; possession of a live raw Bearer or invite token is still sufficient for its allowed action. CogDoc supports one configured enterprise OIDC provider, explicit account linking, and workspace-scoped SCIM provisioning. It still does not provide email delivery, password-reset email, or CogDoc-enforced MFA; enforce MFA, device posture, and conditional access at the IdP without bypassing CogDoc session and ACL checks.

The singular `COGDOC_API_KEY` is an outbound CLI/compatibility-client credential, not an API-side allowlist. The Next.js workspace uses human browser sessions and does not consume it. Do not set this variable on a shared Streamlit compatibility deployment: browsers without their own human session would operate as that service principal. Configure server-side machine identities with persistent service accounts, `COGDOC_API_KEYS`, or `COGDOC_API_PRINCIPALS`; reserve the singular variable for a trusted single-user console or dedicated automation client.
