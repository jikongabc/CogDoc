...
function IntegrationsWorkspace() {
  ...
  const requestedActiveTab = integrationTabs.includes(requestedTab as (typeof integrationTabs)[number]) ? requestedTab! : "connections";
  const activeTab = canManage || ["connections", "jobs"].includes(requestedActiveTab) ? requestedActiveTab : "connections";
  ...
  const credentials = useQuery({ queryKey: ["sources", "credentials", kbId], queryFn: () => controlApi.connectorCredentials(kbId), enabled: canManage && Boolean(kbId), retry: false });
  const credentialEvents = useQuery({ queryKey: ["sources", "credential-events", kbId], queryFn: () => controlApi.connectorCredentialEvents(kbId), enabled: canManage && Boolean(kbId), retry: false });
  const catalog = useQuery({ queryKey: ["sources", "catalog", kbId], queryFn: () => controlApi.sourceCatalog(kbId, true), enabled: canManage && Boolean(kbId) });
  const artifactUsage = useQuery({ queryKey: ["sources", "artifact-usage", kbId], queryFn: () => controlApi.sourceArtifactUsage(kbId), enabled: canManage && Boolean(kbId), retry: false });
  ...
  const versions = useQuery({ queryKey: ["sources", "versions", kbId, activeSourceId], queryFn: () => controlApi.sourceVersions(kbId, activeSourceId), enabled: canManage && Boolean(kbId && activeSourceId) });
  ...
  if (knowledgeBases.isError) {
    return <div className="min-h-full"><PageHeader eyebrow="External data" title="数据接入" description="连接外部系统，并将内容同步为 CogDoc 可检索的知识库文档。" /><div className="p-4 md:p-6"><QueryState error={knowledgeBases.error} onRetry={() => void knowledgeBases.refetch()} errorTitle="无法读取知识库" /></div></div>;
  }
  ...
  return <div className="min-h-full"><PageHeader eyebrow="External data" title="数据接入" description="连接外部系统，并将内容同步为 CogDoc 可检索的知识库文档。" meta={kbId ? <Badge>{kbId}</Badge> : undefined} actions={kbId && canManage ? <><OAuthDialog kbId={kbId} connections={connectionRows} /><CredentialDialog kbId={kbId} /><ConnectionDialog kbId={kbId} credentials={credentialRows} /></> : undefined} /><div className="p-4 md:p-6">
    ...
    <Tabs value={activeTab} onValueChange={(value) => updateLocation(kbId, value)}><div className="mb-4 flex items-end justify-between"><TabsList><TabsTrigger value="connections">连接器 <Badge className="ml-1">{connectionRows.length}</Badge></TabsTrigger>{canManage ? <TabsTrigger value="catalog">来源目录 <Badge className="ml-1">{sourceRows.length}</Badge></TabsTrigger> : null}<TabsTrigger value="jobs">同步任务 <Badge className="ml-1">{jobRows.length}</Badge></TabsTrigger>{canManage ? <><TabsTrigger value="credentials">凭据 <Badge className="ml-1">{credentialRows.length}</Badge></TabsTrigger><TabsTrigger value="credential-audit">凭据审计</TabsTrigger></> : null}</TabsList><Button variant="ghost" size="icon" onClick={refreshAll} aria-label="刷新数据接入"><RefreshCw className={cn("size-4", connections.isFetching && "animate-spin")} /></Button></div>
    <TabsContent value="connections"><section className="overflow-hidden border border-border bg-surface"><QueryState pending={connections.isPending} error={connections.error} onRetry={() => void connections.refetch()} />{health.isError ? <div role="alert" className="flex items-center justify-between gap-3 border-b border-warning/30 bg-warning-subtle px-3 py-2 text-xs text-warning"><span>无法读取连接器健康状态。</span><Button variant="ghost" size="compact" onClick={() => void health.refetch()}>重试</Button></div> : null}{connections.data && !connectionRows.length ? <EmptyState icon={Cable} compact title="没有来源连接器" description="添加本地目录、Git、URL、Notion、Confluence、SharePoint、Zotero 或 S3。" action={canManage ? <ConnectionDialog kbId={kbId} credentials={credentialRows} /> : undefined} /> : <div className="divide-y divide-border">{connectionRows.map((row) => { const id = textValue(row.connection_id, ""); const h = healthByConnection.get(id); const enabled = Boolean(row.enabled); const name = textValue(row.name, id); return <div key={id} className="grid grid-cols-[minmax(0,1fr)_130px_120px_auto] items-center gap-4 px-4 py-3 text-[13px]"><div className="min-w-0"><div className="flex items-center gap-2"><p className="truncate font-medium">{name}</p><Badge>{textValue(row.connector_type)}</Badge></div><p className="mt-1 truncate font-mono text-[10px] text-muted-foreground">{id}</p></div><StatusBadge status={textValue(h?.status, health.isSuccess ? (enabled ? "ready" : "disabled") : "unknown")} /><span className="text-xs text-muted-foreground">{textValue(row.credential_source, "无凭据")}</span>{canManage ? <div className="flex gap-1"><Button variant="ghost" size="icon" disabled={!enabled} loading={sync.isPending && sync.variables === id} onClick={() => sync.mutate(id)} aria-label="立即同步"><Play className="size-4" /></Button><Button variant="ghost" size="icon" onClick={() => toggle.mutate({ id, enabled: !enabled })} aria-label={enabled ? "停用连接器" : "启用连接器"}><Unplug className="size-4" /></Button><ConfirmDeleteButton title="删除连接器" description={`将删除“${name}”的配置；已有同步审计与来源版本仍按后端保留策略处理。`} loading={deleteConnection.isPending && deleteConnection.variables === id} onConfirm={() => deleteConnection.mutate(id)} /></div> : null}</div>; })}</div>}</section></TabsContent>
    <TabsContent value="catalog"><QueryState pending={artifactUsage.isPending} error={artifactUsage.error} onRetry={() => void artifactUsage.refetch()} />{artifactUsage.isSuccess ? <div className="mb-3 grid gap-px overflow-hidden border border-border bg-border sm:grid-cols-4">{[["活动版本", numberValue(usageRow.active_versions)], ["活动原件", numberValue(usageRow.active_bytes)], ["恢复区版本", numberValue(usageRow.trash_versions)], ["恢复区原件", numberValue(usageRow.trash_bytes)]].map(([label, value], index) => <div key={String(label)} className="bg-surface px-3 py-2"><p className="text-[10px] text-muted-foreground">{label}</p><p className="mt-0.5 font-mono text-xs">{index % 2 ? `${Number(value).toLocaleString()} bytes` : value}</p></div>)}</div> : null}<div className="grid gap-4 xl:grid-cols-[minmax(360px,0.9fr)_minmax(420px,1.1fr)]"><section className="overflow-hidden border border-border bg-surface"><QueryState pending={catalog.isPending} error={catalog.error} onRetry={() => void catalog.refetch()} />{catalog.data && !sourceRows.length ? <EmptyState icon={Route} compact title="来源目录为空" description="连接器完成同步后，会生成可追踪的来源和版本记录。" /> : <div className="divide-y divide-border">{sourceRows.map((row) => { const id = textValue(row.source_id, ""); return <button key={id} onClick={() => setSelectedSourceId(id)} className={cn("w-full px-4 py-3 text-left hover:bg-surface-subtle", activeSourceId === id && "bg-primary-subtle")}><div className="flex items-center justify-between gap-2"><p className="truncate text-[13px] font-medium">{textValue(row.title, textValue(row.name, id))}</p><StatusBadge status={textValue(row.health_status, textValue(row.status, "ready"))} /></div><p className="mt-1 truncate font-mono text-[10px] text-muted-foreground">{textValue(row.origin_uri, id)}</p></button>; })}</div>}</section><section className="border border-border bg-surface"><div className="border-b border-border px-4 py-3"><h3 className="text-[13px] font-semibold">版本航迹</h3><p className="text-[11px] text-muted-foreground">{activeSourceId || "选择来源"}</p></div><QueryState pending={Boolean(activeSourceId) && versions.isPending} error={versions.error} onRetry={() => void versions.refetch()} />{versions.data ? (versionRows.length ? <><SourceVersionTools key={activeSourceId} kbId={kbId} sourceId={activeSourceId} versions={versionRows} /><div className="divide-y divide-border">{versionRows.map((row) => { const versionId = textValue(row.version_id); const available = Boolean(row.artifact_available ?? true); return <div key={versionId} className="px-4 py-3"><div className="flex items-center justify-between gap-2"><p className="font-mono text-[11px] font-medium">{versionId}</p><div className="flex items-center gap-1"><StatusBadge status={textValue(row.artifact_status, available ? "ready" : "metadata_only")} />{available ? <Button variant="ghost" size="icon" onClick={() => downloadVersion.mutate({ sourceId: activeSourceId, versionId, name: textValue(activeSource?.display_name, textValue(activeSource?.name, `${activeSourceId}-${versionId}`)) })} aria-label="下载来源版本"><Download className="size-4" /></Button> : null}</div></div><p className="mt-1 text-[11px] text-muted-foreground">{textValue(row.created_at, textValue(row.fetched_at))} · {textValue(row.byte_size, textValue(row.size_bytes, "未知大小"))} bytes</p></div>; })}</div></> : <EmptyState icon={DatabaseZap} compact title="没有版本记录" description="每次来源内容变化都会形成不可变版本。" />) : null}</section></div></TabsContent>
    <TabsContent value="jobs"><section className="overflow-hidden border border-border bg-surface"><QueryState pending={jobs.isPending} error={jobs.error} onRetry={() => void jobs.refetch()} />{jobs.data ? (jobRows.length ? <div className="divide-y divide-border">{jobRows.map((row) => { const id = textValue(row.job_id); const status = textValue(row.status, "pending"); const errorMessage = textValue(row.error_message, ""); return <div key={id} className="grid grid-cols-[minmax(0,1fr)_110px_180px_auto] items-center gap-3 px-4 py-3 text-[13px]"><div className="min-w-0"><p className="font-medium">{textValue(row.connector_type)} 同步</p><p className="font-mono text-[10px] text-muted-foreground">{id}</p>{errorMessage ? <p className="mt-1 truncate text-[11px] text-error" title={errorMessage}>{errorMessage === "provider HTTP 401" ? "提供方认证失败（HTTP 401），请检查或轮换凭据。" : errorMessage}</p> : null}</div><StatusBadge status={status} /><span className="text-xs text-muted-foreground">{textValue(row.documents_fetched, "0")} 文档 · 尝试 {textValue(row.attempt, "0")}{row.finished_at ? ` · ${formatDateTime(row.finished_at as string | number)}` : ""}</span>{status === "dead_letter" && canManage ? <Button size="compact" onClick={() => replay.mutate(id)} loading={replay.isPending && replay.variables === id}><RotateCcw className="size-3.5" />重放</Button> : <span />}</div>; })}</div> : <EmptyState icon={RefreshCw} compact title="没有同步任务" description="从连接器行启动一次同步。" />) : null}</section></TabsContent>
