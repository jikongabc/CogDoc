    ...
    <div className="min-w-0">
      <div className="flex min-h-[76px] items-start justify-between gap-4 border-b border-border px-5 py-3.5">
        ...
        <div className="flex shrink-0 items-center gap-1.5">
          {canWrite && ["draft", "pending"].includes(status) ? <Button size="compact" onClick={() => generate.mutate()} loading={generate.isPending}><WandSparkles className="size-3.5" />生成计划</Button> : null}
          {canWrite && ["draft", "pending", "planned"].includes(status) ? <Button variant="primary" size="compact" onClick={() => action.mutate("start")} loading={action.isPending}><Play className="size-3.5" />开始</Button> : null}
          {canWrite && status === "running" ? <Button size="compact" onClick={() => action.mutate("pause")}><Pause className="size-3.5" />暂停</Button> : null}
          {canWrite && status === "paused" ? <Button variant="primary" size="compact" onClick={() => action.mutate("resume")}><Play className="size-3.5" />继续</Button> : null}
          {canWrite && status === "evidence_ready" ? <><Button size="compact" onClick={() => action.mutate("refresh")} loading={action.isPending}><RefreshCw className="size-3.5" />重新取证</Button><Button variant="primary" size="compact" onClick={() => action.mutate("generate")} loading={action.isPending}><WandSparkles className="size-3.5" />生成报告</Button></> : null}
          {canWrite && ["pending", "running", "paused", "generating"].includes(status) ? <Button variant="ghost" size="compact" onClick={() => action.mutate("cancel")}><Square className="size-3.5" />取消</Button> : null}
          {canPublish && status === "completed" && reviewStatus === "approved" && !textValue(row.published_at, "") ? <Button variant="primary" size="compact" onClick={() => publish.mutate()} loading={publish.isPending}><Send className="size-3.5" />发布</Button> : null}
        </div>
      </div>
      <Tabs defaultValue="plan">
        <TabsList className="h-10 gap-6 px-5"><TabsTrigger value="plan">计划</TabsTrigger><TabsTrigger value="report">报告</TabsTrigger>{canReview ? <TabsTrigger value="review">审阅</TabsTrigger> : null}<TabsTrigger value="provenance">证据链</TabsTrigger><TabsTrigger value="activity">运行状态</TabsTrigger></TabsList>
        <TabsContent value="plan" className="p-5">{sections.length ? <><div className="mb-3 flex items-center justify-between gap-3"><p className="text-xs text-muted-foreground">执行前可修订章节、研究问题和逐条检索表达。</p>{canWrite && status === "planned" ? <EditResearchPlanDialog jobId={jobId} revision={revision} sections={sections} onSaved={invalidate} /> : null}</div><div className="border border-border">{sections.map((section, index) => <div key={textValue(section.section_id, String(index))} className="border-b border-border p-3 last:border-b-0"><div className="flex items-start gap-3"><span className="mt-0.5 font-mono text-[10px] text-muted-foreground">{String(index + 1).padStart(2, "0")}</span><div><p className="text-[13px] font-medium">{textValue(section.title, `章节 ${index + 1}`)}</p><p className="mt-1 text-xs leading-5 text-muted-foreground">{textValue(section.research_question, textValue(section.objective, textValue(section.description, "等待定义证据要求")))}</p><p className="mt-1 text-[11px] text-muted-foreground">{Array.isArray(section.evidence_requirements) ? `${section.evidence_requirements.length} 条证据需求` : "尚未定义证据需求"}</p></div></div></div>)}</div></> : <EmptyState icon={FileSearch} compact title="尚未生成研究计划" description="生成计划后可检查章节和原子证据要求。" action={canWrite ? <Button onClick={() => generate.mutate()} loading={generate.isPending}><WandSparkles className="size-4" />生成计划</Button> : undefined} />}</TabsContent>
        <TabsContent value="report" className="p-5"><QueryState pending={status === "completed" && report.isPending} error={report.error} onRetry={() => void report.refetch()} label="正在读取研究报告" />{!report.error && !(status === "completed" && report.isPending) ? (reportText ? <><div className="mb-4 flex justify-end"><Button onClick={downloadReport}>下载 Markdown</Button></div><article className="answer-markdown mx-auto max-w-3xl text-[14px] leading-6"><ReactMarkdown remarkPlugins={[remarkGfm]} skipHtml>{reportText}</ReactMarkdown></article></> : <EmptyState icon={FlaskConical} compact title="报告尚未生成" description="研究完成取证和章节生成后，报告会出现在这里供审阅。" />) : null}</TabsContent>
        {canReview ? <TabsContent value="review"><ResearchReview jobId={jobId} revision={revision} sections={sections} onSaved={invalidate} /></TabsContent> : null}
        <TabsContent value="provenance" className="p-5"><QueryState pending={provenance.isPending} error={provenance.error} onRetry={() => void provenance.refetch()} label="正在读取证据链" />{provenance.data ? (provenanceRows.length ? <div className="divide-y divide-border border border-border">{provenanceRows.map((item, index) => <div key={textValue(item.evidence_id, String(index))} className="grid grid-cols-[32px_minmax(0,1fr)_auto] gap-3 px-3 py-3"><span className="font-mono text-[10px] text-muted-foreground">[{index + 1}]</span><div className="min-w-0"><p className="truncate text-[13px] font-medium">{textValue(item.source, textValue(item.title, "证据来源"))}</p><p className="mt-1 line-clamp-2 text-xs leading-5 text-muted-foreground">{textValue(item.text, textValue(item.excerpt, textValue(item.requirement, "来源元数据已记录")))}</p></div><StatusBadge status={textValue(item.status, "verified")} /></div>)}</div> : <EmptyState icon={CheckCircle2} compact title="暂无证据链记录" description="Research 开始检索后，这里会显示要求、来源和核验状态。" />) : null}</TabsContent>
...
export default function ResearchPage() {
  ...
  if (knowledgeBases.isError) {
    return <div className="min-h-full"><PageHeader eyebrow="Evidence research" title="研究" description="从目标到计划、取证、审阅与发布，完整保留 Research 的可暂停、可恢复生命周期。" /><div className="p-4 md:p-6"><QueryState error={knowledgeBases.error} onRetry={() => void knowledgeBases.refetch()} errorTitle="无法读取知识库" /></div></div>;
  }
  return (
    <div className="flex min-h-full flex-col">
      <PageHeader eyebrow="Evidence research" title="研究" description="从目标到计划、取证、审阅与发布，完整保留 Research 的可暂停、可恢复生命周期。" actions={activeKb && canWrite ? <CreateResearchDialog kbId={activeKb} onCreated={(id) => { selectJob(id); void jobs.refetch(); }} /> : undefined} />
