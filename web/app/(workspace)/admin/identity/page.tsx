...
export default function IdentityPage() {
  ...
  const identities = useQuery({ queryKey: ["admin", "oidc-identities"], queryFn: controlApi.oidcIdentities, enabled: canManage && Boolean(workspaceId) && Boolean(config.data?.oidc_enabled), retry: false });
  const policy = useQuery({ queryKey: ["admin", "oidc-policy", workspaceId], queryFn: () => controlApi.oidcPolicy(workspaceId), enabled: canManage && Boolean(workspaceId) && Boolean(config.data?.oidc_enabled), retry: false });
  const scim = useQuery({ queryKey: ["admin", "scim", workspaceId], queryFn: () => controlApi.scimStatus(workspaceId), enabled: canManage && Boolean(workspaceId) && Boolean(config.data?.scim_enabled), retry: false });
  ...
  return (
    <AdminPageFrame>
      ...
      {!workspaceId ? <EmptyState icon={Fingerprint} title="账号认证未启用" description="本地兼容模式不创建企业身份目录。启用账号认证后可配置 OIDC 与 SCIM。" /> : <div className="space-y-6 p-4 md:p-6">
        ...
        <section className="border border-border bg-surface"><div className="flex h-11 items-center justify-between border-b border-border px-4"><div><h3 className="text-[13px] font-semibold">SCIM 自动预配</h3><p className="text-[11px] text-muted-foreground">服务提供方能力与工作区注册状态</p></div><StatusBadge status={!config.data?.scim_enabled ? "disabled" : scim.data ? textValue(scimRow.status, "active") : scim.isError ? "unknown" : "pending"} /></div>{config.data?.scim_enabled ? <QueryState pending={scim.isPending} error={scim.error} onRetry={() => void scim.refetch()} /> : null}{!config.data?.scim_enabled || scim.data ? <><div className="grid gap-px bg-border sm:grid-cols-3">{[["部署能力", config.data?.scim_enabled ? "已启用" : "未启用"], ["工作区状态", textValue(scimRow.status, "—")], ["资源端点", textValue(scimRow.base_url, "由服务端提供")]].map(([label, value]) => <div key={String(label)} className="bg-surface p-4"><p className="text-[11px] text-muted-foreground">{label}</p><p className="mt-1 text-[13px] font-medium">{String(value)}</p></div>)}</div>{config.data?.scim_enabled && typeof scimRow.base_url === "string" ? <div className="border-t border-border px-4 py-3"><Button asChild variant="ghost" size="compact"><a href={scimRow.base_url} target="_blank" rel="noreferrer">查看 SCIM 端点<ExternalLink className="size-3.5" /></a></Button></div> : null}</> : null}</section>
      </div>}
    </AdminPageFrame>
  );
}
