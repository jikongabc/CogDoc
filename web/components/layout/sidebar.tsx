...
function RailSection({ title, action, children }: { title: string; action?: React.ReactNode; children: React.ReactNode }) {
  return (
    <section className="px-2 pb-3 pt-2">
      <div className="mb-1.5 flex min-h-7 items-center justify-between gap-2 px-2">
...
export function Sidebar({ className, forceExpanded = false }: { className?: string; forceExpanded?: boolean }) {
  ...
  const selectKnowledgeBase = (targetKb: string) => {
    ...
    if (pathname.startsWith("/integrations")) {
      router.push(`/integrations?kb=${encodeURIComponent(targetKb)}`);
      return;
    }
  ...
  if (collapsed) {
    return (
      <aside className={cn("flex h-dvh w-14 shrink-0 flex-col items-center border-r border-border/80 bg-sidebar py-2", className)}>
        <Link href="/home" className="mb-2 flex size-8 items-center justify-center rounded-[8px] bg-foreground text-white shadow-[var(--shadow-edge)]" aria-label="CogDoc 工作台"><BookOpenCheck className="size-[17px]" /></Link>
  ...
  return (
    <aside className={cn("flex h-dvh w-[272px] shrink-0 flex-col border-r border-border/80 bg-sidebar", className)}>
      <div className="flex h-14 shrink-0 items-center justify-between px-3">
        <Link href="/home" className="flex items-center gap-2" aria-label="CogDoc 工作台">
          <span className="flex size-8 items-center justify-center rounded-[8px] bg-foreground text-white shadow-[var(--shadow-edge)]"><BookOpenCheck className="size-[17px]" /></span>
          <span><span className="block text-sm font-semibold tracking-[-0.02em]">CogDoc</span><span className="block text-[10px] leading-3 text-muted-foreground">企业知识工作台</span></span>
        </Link>
      ...
      <div className="shrink-0 px-2 pb-2"><WorkspaceSwitcher /></div>

      <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain">
        <RailSection title="知识库">
          {knowledgeBases.isPending ? <div className="flex h-9 items-center gap-2 px-2 text-xs text-muted-foreground"><Spinner size="sm" />正在读取知识库</div> : knowledgeBases.isError ? <div role="alert" className="border-l-2 border-error bg-error-subtle px-2 py-2 text-xs text-error">无法读取知识库</div> : knowledgeBases.data?.length ? (
            <select
              aria-label="选择知识库"
              value={kbId}
              onChange={(event) => selectKnowledgeBase(event.target.value)}
              className="h-9 w-full rounded-[7px] border border-border-strong bg-surface px-2.5 text-[13px] font-medium outline-none focus-visible:ring-2 focus-visible:ring-primary/60"
            >
        ...
        {kbId ? (
          <>
            <RailSection title="对话" action={<Button variant="ghost" size="compact" onClick={() => newConversation()}><MessageSquarePlus className="size-3.5" />新对话</Button>}>
              {sessions.isPending ? <div className="flex h-12 items-center justify-center gap-2 text-xs text-muted-foreground"><Spinner size="sm" />正在读取</div> : sessions.isError ? <button className="w-full border-l-2 border-error bg-error-subtle px-2 py-2 text-left text-xs text-error" onClick={() => void sessions.refetch()}><span className="block font-medium">无法读取对话</span><span className="mt-0.5 block opacity-80">点击重试</span></button> : visibleSessions.length ? (
                <nav className="space-y-0.5" aria-label="对话记录">
                  {visibleSessions.slice(0, 30).map((session) => (
                    <div key={session.session_id} className={cn("group flex items-center rounded-[7px] transition-colors hover:bg-surface-subtle", session.session_id === activeSessionId && "bg-surface-subtle")}>
                      <Link href={`/knowledge/${encodeURIComponent(kbId)}/chat/${encodeURIComponent(session.session_id)}`} className={cn("min-w-0 flex-1 px-2.5 py-2 text-[13px] text-muted-foreground hover:text-foreground", session.session_id === activeSessionId && "font-medium text-foreground")}>
            ...
            <RailSection title="文档" action={canWrite ? (
              ...
              {documents.isPending ? <div className="flex h-12 items-center justify-center gap-2 text-xs text-muted-foreground"><Spinner size="sm" />正在读取</div> : documents.isError ? <button className="w-full border-l-2 border-error bg-error-subtle px-2 py-2 text-left text-xs text-error" onClick={() => void documents.refetch()}>读取失败，点击重试</button> : documents.data?.length ? (
                <div className="space-y-0.5">
                  {documents.data.map((document) => (
                    <div key={document.name} className="group flex min-h-8 items-center rounded-[7px] px-2.5 transition-colors hover:bg-surface-subtle">
              ...
              <label className="mt-2 flex min-h-8 cursor-pointer items-center gap-2 rounded-[7px] px-2.5 text-xs text-muted-foreground transition-colors hover:bg-surface-subtle hover:text-foreground">
      ...
      <div className="shrink-0 border-t border-border/80 p-2">
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <button className="flex h-11 w-full items-center gap-2.5 rounded-[8px] px-2 text-left transition-colors hover:bg-surface-subtle">
