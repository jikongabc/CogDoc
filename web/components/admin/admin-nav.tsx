...
export function AdminNav() {
  ...
  const visibleSections = canManage ? sections : sections.filter(({ href }) => href === "/admin/security");
  return (
...
export function AdminPageFrame({ children, allowAccountUser = false }: { children: React.ReactNode; allowAccountUser?: boolean }) {
  const canManage = useAnyPermission(["manage_access", "manage_tenant"]);
  if (!canManage && !allowAccountUser) {
